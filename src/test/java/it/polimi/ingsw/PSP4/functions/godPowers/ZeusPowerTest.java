...
public class ZeusPowerTest {
    ...
    public void powerNotUsed_standardBuild() {
        ...
        workers.put(players.get(0), Arrays.asList(new Coordinates(0, 0), new Coordinates(2, 2)));
        workers.put(players.get(1), Arrays.asList(new Coordinates(1, 2), new Coordinates(0, 2)));
        ...
        List<Coordinates> expectedMoveOptions = Collections.singletonList(new Coordinates(1, 1));
    ...
    @Test
    public void powerUsed_buildsUnderHimself(){
        int numPlayer = 2;
        List<String> players = Random.playerList(numPlayer);
        Map<String, String> gods = new LinkedHashMap<>();
        gods.put(players.get(0), godName);
